...
function validate(md: string, author: Author) {
	...
	if (!list.ok) errors.push(`Expected one practical list with exactly 3 items. Found: ${list.items}`)

	const introClosing = hasIntroAndClosing(md)
	if (!introClosing.introOk) errors.push("Intro paragraph missing/too short (before first ##).")
	if (!introClosing.closingOk) errors.push("Closing paragraph missing/too short (tail of post).")
	...
	if (author === "yuliaM") {
		if (emojis < 8 || emojis > 14) errors.push(`yuliaM: expected 8-14 emojis. Found: ${emojis}`)
	}
	...
	if (author === "Svetlana") {
		...
		if (!lc.ok) errors.push(`Svetlana: expected exactly 4 lowercase-after-period cases. Found: ${lc.count}`)
